...
// Particles are a landing-page accent only: fully visible through the Hero
// section, faded out entirely by the time the Skills section is reached so
// they never clutter the later, more detailed sections.
const FADE_END_PROGRESS = 2 / 7;

export function CodeParticles({ scrollProgress }) {
  const group = useRef();
  ...
  useEffect(() => {
    ...
    const unsubscribe = scrollProgress.on('change', (value) => {
      setOpacity(THREE.MathUtils.clamp(1 - value / FADE_END_PROGRESS, 0, 1));
    });
    return unsubscribe;
  }, [scrollProgress]);
  ...
  const particles = useMemo(() => {
    ...
    return Array.from({ length: 8 }).map(() => {
      ...
      return {
        ...
        color: ['#ff8a65', '#22d3ee', '#4caf50', '#ffeb3b', '#e0e0e0'][Math.floor(Math.random() * 5)]
      };
    });
  }, []);

  useFrame(() => {
    if (group.current && opacity > 0.01) {
